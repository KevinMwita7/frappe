	...
	def set_value(self, dt, dn, field, val=None, modified=None, modified_by=None,
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		if dn and dt!=dn:
			...
			# self.sql('''update `tabSingles` set doctype='System Settings', field='System Settings', value='scheduler_last_event' where field in ({0}) and doctype=%s'''.format(', '.join(['%s']*len(keys))), list(keys) + [dt], debug=debug)
